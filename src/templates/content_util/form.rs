...
mod imp {
    use gtk::glib::{
        ...
        ParamSpec, ParamSpecBoolean, ParamSpecString, Properties,
    };
    ...
    impl ObjectInterface for FormObject {
        ...
        fn properties() -> &'static [ParamSpec] {
            ...
            PROPERTIES.get_or_init(|| {
                vec![
                    ParamSpecBoolean::builder("required").readwrite().build(),
                    ParamSpecString::builder("label").readwrite().build(),
                ]
            })
        }
...
unsafe impl<T: FormObjectImpl> IsImplementable<T> for FormObject {
    ...
    fn interface_init(iface: &mut glib::Interface<Self>) {
        ...
        fn is_valid_trampoline<T: ObjectSubclass + FormObjectImpl>(obj: &FormObject) -> bool {
            let this = obj
                .dynamic_cast_ref::<<T as ObjectSubclass>::Type>()
                .unwrap()
                .imp();
        ...
        fn set_value_trampoline<T: ObjectSubclass + FormObjectImpl>(
            obj: &FormObject,
            value: Value,
        ) {
            let this = obj
        ...
        fn display_error_trampoline<T: ObjectSubclass + FormObjectImpl>(
            obj: &FormObject,
...
pub trait FormObjectExt: IsA<FormObject> + IsA<gtk::Widget> + 'static {
    ...
    fn construct_form_obj(&self) {
        let ancestor: FormManager = self
            .ancestor(FormManager::static_type())
            .and_downcast()
            .expect("Could not get FormManager.");
