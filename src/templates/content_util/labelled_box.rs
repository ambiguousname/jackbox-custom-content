...
use gtk::glib::{derived_properties, BorrowedObject, Properties, Value};
...
impl WidgetImpl for imp::LabelledBox {
    fn realize(&self) {
        ...
        value_obj.connect_notify(
            Some(&self.value_property.borrow().clone()),
            move |child, _| {
                let parent = child
                    .ancestor(LabelledBox::static_type())
                    .and_downcast::<LabelledBox>()
...
impl LabelledBox {
    ...
    pub fn value(&self) -> Value {
        self.value_obj()
            .property(&self.imp().value_property.borrow().clone())
    }

    pub fn set_value(&self, value: Value) {
        self.value_obj()
            .set_property(&self.imp().value_property.borrow().clone(), value);
    }
...
unsafe impl<T: LabelledBoxImpl> IsSubclassable<T> for LabelledBox {
    fn class_init(class: &mut glib::Class<Self>) {
        ...
        fn get_value_obj_trampoline<T: ObjectSubclass + LabelledBoxImpl>(
            obj: &LabelledBox,
        ) -> gtk::Widget {
            let this = obj
