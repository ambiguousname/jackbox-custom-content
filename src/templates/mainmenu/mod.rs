...
use crate::{
    mod_manager::{mod_store::ModStore, ModManager},
    quick_template,
};
...
impl MainMenuWindow {
    ...
    fn setup_actions(&self) {
        ...
        let help_action = ActionEntry::builder("help")
            .activate(|window: &MainMenuWindow, _, _| {
                let result =
                    open::that("https://github.com/ambiguousname/jackbox-custom-content/wiki");
                if result.is_err() {
                    let dlg = AlertDialog::builder()
        ...
        let about_action = ActionEntry::builder("about")
            .activate(|window: &MainMenuWindow, _, _| {
                let authors: Vec<&str> = option_env!("CARGO_PKG_AUTHORS")
                    .unwrap()
                    .split(",")
                    .collect();
    ...
    fn mod_manager(&self) -> &ModManager {
        self.imp()
            .mod_manager
            .get_or_init(|| ModManager::new(self.clone()))
    }
    ...
    fn setup_stack(&self) {
        self.imp()
            .mod_stack
            .connect_notify(Some("visible-child"), |this, _| {
                MainMenuWindow::stack_changed(this);
            });
    ...
    pub fn add_mod_to_stack(&self, name: String, mod_store: &ModStore) {
        self.imp()
            .mod_stack
            .add_titled(mod_store, Some(name.as_str()), name.as_str());
    ...
    pub fn remove_mod_from_stack(&self, name: String) {
        ...
        if (self.imp().mod_stack.pages().n_items() - 1 > 0) {
            self.imp()
                .mod_stack
                .set_visible_child(&child.next_sibling().or(child.prev_sibling()).unwrap());
        }
    ...
    pub fn add_content_to_mod(&self, content: crate::content::Content) {
        let mod_name = self
            ...
            .expect("Nothing visible on the stack.");
        self.mod_manager()
            .add_content_to_mod(mod_name.to_string(), content);
    }
    ...
    fn preferences_window(&self) -> &PreferencesWindow {
        self.imp()
            .preferences_window
            .get_or_init(|| PreferencesWindow::new(self, self.config()))
    }
    // endregion
    ...
    fn content_creation_dialog(&self) -> &ContentCreationDialog {
        self.imp()
            ...
            .get_or_init(|| ContentCreationDialog::new(self))
    }
