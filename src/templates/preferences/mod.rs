use gtk::{
    gio::{self, Settings},
    glib::{clone, derived_properties},
    AlertDialog, DialogError, FileDialog, Switch, Window,
};
...
impl PreferencesWindow {
    ...
    fn settings(&self) -> &Settings {
        self.imp()
            .app_settings
            .get()
            .expect("Could not get app settings.")
    }
    ...
    fn handle_dark_mode(&self, val: bool) -> bool {
        ...
        self.default_settings()
            .set_gtk_application_prefer_dark_theme(val);
    ...
    fn handle_folder_set(&self) {
        let parent: MainMenuWindow = self
            .transient_for()
            .and_downcast()
    ...
    fn set_mod_folder(&self, result: Result<gio::File, glib::Error>) -> Result<String, String> {
        if result.is_ok() {
            ...
            let folder_set = self.settings().set_string(
                "mods-folder",
